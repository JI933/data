...
for i, region in enumerate(regions):
    ...
    bars = plt.bar(['入门岗', '进阶岗', '专家岗'], values, bottom=bottom,
                   color=colors_region[i], alpha=0.8, label=region, width=0.55, edgecolor='white')
    for j, bar in enumerate(bars):
        height = bar.get_height()
        if height > 5:
            plt.text(bar.get_x() + bar.get_width()/2, bottom[j] + height/2,
                     f'{height:.0f}%', ha='center', va='center', fontsize=9, color='white')
    bottom += values
...
for i, tier in enumerate(city_tiers):
    ...
    bars = plt.bar(['入门岗', '进阶岗', '专家岗'], values, bottom=bottom,
                   color=colors_tier[i], alpha=0.8, label=tier, width=0.55, edgecolor='white')
    for j, bar in enumerate(bars):
        height = bar.get_height()
        if height > 5:
            plt.text(bar.get_x() + bar.get_width()/2, bottom[j] + height/2,
